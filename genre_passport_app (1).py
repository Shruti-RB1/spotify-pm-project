...
def generate_journey(taste_input, adventure):
    ...
    if not client:
        return None
    # ── FIX: strengthened prompt ──────────────────────────────────────────
    # 1. Familiarity must now be strictly increasing 1→5, no duplicates
    #    (previously: "progressively further" was too loose, AI repeated values)
    # 2. Added explicit anti-hallucination rule requiring correct artist
    #    attribution, since a prior test returned "Ravi Shankar — Norwegian
    #    Wood" — that song is by The Beatles, not Ravi Shankar. Also lowered
    #    temperature slightly (0.7 → 0.5) to reduce factual drift while
    #    keeping enough variety across different taste inputs.
...
Rules:
- Familiarity values across the 5 songs MUST be strictly increasing: 1, 2, 3, 4, 5 — exactly one song per value, no duplicates and no skipped numbers.
- Song 1 must feel very familiar (familiarity: 1)
- Song 5 reaches the adventure level destination (familiarity: 5)
- CRITICAL — artist accuracy: only credit a song to the artist who actually recorded/released it. Do not attribute a song to an artist merely associated with its style, genre, or a collaborator (e.g. do not credit a sitar-influenced Western song to an Indian classical musician who did not perform it). If you are not fully certain of the correct artist for a specific real song, choose a different song you are certain about instead.
...
# ── INPUT ──────────────────────────────────────────────────────────────────────
    ...
    # ── FIX: validation — blocks empty/too-short input before calling AI ──
    if st.button("✨ Generate my discovery journey", type="primary", use_container_width=True):
        ...
        elif len(taste.strip()) < 10:
            st.warning("⚠️ A little more detail helps — try mentioning an artist, genre, or specific mood.")
        else:
