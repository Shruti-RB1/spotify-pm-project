...
# ── INPUT ──────────────────────────────────────────────────────────────────────
    ...
    # Previously this block called generate_journey(taste, adv) with zero
    # check on `taste`. An empty field silently sent an empty string to the
    # AI, which produced a generic, unrelated journey (e.g. "Indie Folk
    # Expansion") with no signal to the user that nothing was actually
    # personalized. Now the button won't proceed without real input.
