...
/* ---- Fix: selectbox + its dropdown menu were light-themed ----
   The dropdown menu renders in a separate overlay (baseweb popover),
   so it needs its own selector — this is likely the "white box" you saw
   if it wasn't the Clear button. */
...
# ── Filter now sits BEFORE the CTA, since the button needs to know
#    which subset of reviews to analyze based on the current selection.
...
with cta_col2:
    st.caption(f"Answers 6 PM research questions · Powered by Groq · Llama 3.3 70B · Free · ~20 seconds")
