...
# ============================================================================
# APP-LIKE THEME — fixes applied here:
# 1. Global dark theme for ALL native widgets (buttons, selectbox, tabs) —
#    previously only .stApp background was dark, so buttons/dropdowns kept
#    Streamlit's default light theme, causing the stray white "Clear results" box.
# 2. Pill-style CTA buttons with glow, matching app aesthetic.
# 3. Native st.tabs() restyled as solid pill tabs instead of thin underline.
# 4. Rounded cards + soft shadows throughout for a native-app feel.
# ============================================================================
st.markdown("""
<style>
...
/* ---- Fix: native buttons were light-themed, causing white box bug ---- */
div.stButton > button {
    background: #1a1a1a;
    color: #e0e0e0;
...
/* Primary CTA (Analyze) gets the glowing pill treatment */
div.stButton > button[kind="primary"] {
    background: linear-gradient(135deg, #1DB954, #17a049);
    color: white;
    border: none;
    border-radius: 999px;
    padding: 14px 28px;
...
/* ---- Selectbox + its dropdown menu: dark theme ----
   The dropdown menu renders in a separate overlay (baseweb popover),
...
/* ---- Toolbar row: filter + Analyze button, aligned like a real toolbar
   instead of a full-width stretched dropdown sitting above a differently-
   sized button. Both controls share equal height and sit on one line. ---- */
.toolbar-row div[data-baseweb="select"] > div {
    border-radius: 10px !important;
    min-height: 44px;
}
.toolbar-row div.stButton > button[kind="primary"] {
    border-radius: 10px !important;
    height: 44px;
    padding: 0 24px;
    white-space: nowrap;
...
/* ---- Fix: st.tabs() default thin underline → solid pill tabs ---- */
...
/* ---- Mobile fix: tabs were clipped mid-word on narrow screens because
   Streamlit's native tab bar defaults to horizontal scroll with no visible
   affordance — it just looked cut off ("Frustrations" showed as "strations",
   "Repeat" showed as just "R"). Wrapping onto multiple rows on small
   screens shows every tab fully instead. ---- */
...
/* ---- Review cards: soft shadow for app-card feel ---- */
/* ---- Fix: widget labels ("Filter by source") were low-contrast ----
   Streamlit's default label color didn't get overridden by our earlier
   dark-theme pass, so it rendered dim/grey-on-dark and was hard to read. */
...
def analyze_reviews(reviews_list):
    ...
    if not client:
        return None
    # ── FIX: was randomly sampling 35 of 50 reviews on EVERY click, which
    #    meant each analysis ran on a different subset — explaining why
    #    sentiment %, headline, and findings changed every time you clicked
    #    "Analyze 50 reviews with AI" even though nothing else changed.
    #    Now it consistently analyzes the full, fixed set of 50 reviews,
    #    matching what the button actually says.
    sample = reviews_list
...
st.markdown("""
<div style="background:rgba(29,185,84,0.15);border:1px solid #1DB954;border-radius:14px;padding:20px 24px;margin-bottom:16px;box-shadow:0 8px 24px rgba(0,0,0,0.4);">
...
# ── Toolbar redesign: filter (compact, ~1/3 width) + Analyze button
#    (remaining width) sit on ONE aligned row instead of a full-width
#    stretched dropdown stacked above a differently-sized button below it.
#    This is how a compact filter+action toolbar should read — one visual
#    unit, not two disconnected elements.
st.markdown('<div class="toolbar-row">', unsafe_allow_html=True)
tb_col1, tb_col2 = st.columns([1, 2])
with tb_col1:
    source = st.selectbox("Filter by source", SOURCES, label_visibility="visible")
with tb_col2:
    shown = REVIEWS if source == "All sources" else [r for r in REVIEWS if r["src"] == source]
    st.markdown('<div style="height:1.6rem;"></div>', unsafe_allow_html=True)  # aligns button with select box, not its label
    button_label = f"✨ Analyze all {len(shown)} reviews with AI" if source == "All sources" else f"✨ Analyze {len(shown)} {source} reviews with AI"
    analyze_now = st.button(button_label, type="primary", use_container_width=True, disabled=(len(shown) == 0))
st.markdown('</div>', unsafe_allow_html=True)

st.caption("Answers 6 PM research questions · Powered by Groq · Llama 3.3 70B · Free · ~20 seconds")

# ── FIX: the filter now actually drives the analysis. Previously the
#    button always analyzed all 50 reviews regardless of the filter
#    selection, so "Filter by source" only changed the review list you
#    could scroll through — it had zero effect on Key Finding, sentiment,
#    Barriers, Root Cause, etc. Now selecting e.g. "Reddit" analyzes only
#    Reddit reviews, so the two panels correlate.
if analyze_now:
    with st.spinner(f"Analyzing {len(shown)} reviews with Llama 3.3... (~20 seconds)"):
        result = analyze_reviews(shown)
        if result:
            st.session_state["result"] = result
            st.session_state["result_source"] = source
            st.session_state["just_analyzed"] = True  # triggers one-time auto-scroll below
            st.rerun()
...
# ── Auto-scroll to results after a fresh analysis completes ────────────
#    Note: this works by injecting a small script into a component iframe
#    that reaches into the parent page to scroll it — a common Streamlit
#    workaround, but not an officially supported API, so it's worth a
#    live check after deploying rather than assuming it behaves identically
#    everywhere. Only fires once per fresh analysis (not on every rerun
#    caused by other clicks like switching tabs), via the just_analyzed flag.
if st.session_state.get("just_analyzed"):
    components.html("""
        <script>
            setTimeout(function() {
                const target = window.parent.document.getElementById('analysis-results');
                if (target) {
...
# ── FIX: mobile UX. Streamlit stacks these two columns VERTICALLY on
#    narrow screens (e.g. phones), in the order they're defined — so the
#    full review list renders before the AI results appear below it.
#    On mobile, someone has to scroll past every review card just to see
#    the analysis. This anchor link is a manual fallback in case
#    auto-scroll doesn't fire in a given environment.
if "result" in st.session_state:
    st.markdown("""
<a href="#analysis-results" style="display:inline-block;color:#1DB954;font-size:13px;font-weight:600;text-decoration:none;margin-bottom:12px;">
📊 Jump to AI analysis results ↓
</a>""", unsafe_allow_html=True)

with left:
    # ── FIX: the summary line said "Showing {len(shown)}" (i.e. "Showing 50")
    #    while only the first 30 review cards actually rendered below —
    #    that mismatch is exactly why the other 20 seemed to vanish.
    #    Now uses a real, adjustable display count with a "Show more" button.
    if "review_display_count" not in st.session_state:
        st.session_state["review_display_count"] = 30

    display_count = min(st.session_state["review_display_count"], len(shown))
    st.markdown(f'<p style="color:#1DB954;font-weight:600;">📊 {len(REVIEWS)} reviews loaded · Showing {display_count} of {len(shown)}</p>', unsafe_allow_html=True)

    for r in shown[:display_count]:
        stars = ("★" * r["rating"] + "☆" * (5 - r["rating"])) if r["rating"] else "—"
        st.markdown(f"""<div class="review-card" style="background:#1e1e1e;border:1px solid #2a2a2a;border-radius:10px;padding:10px;margin-bottom:8px;">
...
<p style="color:#ddd;font-size:12px;margin:0;line-height:1.6;">{r['text']}</p>
    ...
    if len(shown) > display_count:
        if st.button(f"Show {min(20, len(shown) - display_count)} more reviews", use_container_width=True):
            st.session_state["review_display_count"] += 20
            st.rerun()

with right:
    st.markdown('<div id="analysis-results"></div>', unsafe_allow_html=True)
    if "result" not in st.session_state:
        st.markdown("""<div style="background:#1a1a1a;border:2px dashed #333;border-radius:14px;padding:3rem;text-align:center;margin-top:1rem;">
<p style="font-size:3rem;margin:0 0 12px;">📊</p>
<p style="color:#ddd;font-size:15px;font-weight:600;margin:0 0 6px;">Click Analyze to run AI analysis</p>
<p style="color:#888;font-size:13px;margin:0;">Answers 6 PM research questions<br>Powered by Groq · Llama 3.3 70B · Free</p>
</div>""", unsafe_allow_html=True)
    else:
        a = st.session_state["result"]

        st.markdown(f"""<div style="background:rgba(29,185,84,0.12);border:1px solid #1DB954;border-radius:12px;padding:14px 18px;margin-bottom:14px;box-shadow:0 4px 14px rgba(0,0,0,0.35);">
<p style="color:#1DB954;font-size:10px;text-transform:uppercase;margin:0 0 5px;">KEY FINDING · {st.session_state.get("result_source", "All sources")}</p>
<p style="color:#1DB954;font-size:17px;font-weight:600;margin:0 0 6px;">{a.get("headline","")}</p>
<p style="color:#1DB954;font-size:12px;opacity:0.85;margin:0;">{a.get("discovery_pct","")}</p>
</div>""", unsafe_allow_html=True)

        # ── FIX: sentiment stats replaced with high-contrast, color-coded
        #    HTML instead of st.metric() (which rendered too dim/muted).
        s = a.get("sentiment", {})
        st.markdown(f"""
        <div style="display:flex;gap:32px;margin:16px 0 24px;">
            <div>
                <div style="color:#ff8a8a;font-size:14px;font-weight:600;">😤 Negative</div>
                <div style="color:#ff5c5c;font-size:36px;font-weight:800;">{s.get('negative',0)}%</div>
            </div>
            <div>
                <div style="color:#e0c96b;font-size:14px;font-weight:600;">😐 Neutral</div>
                <div style="color:#f0d878;font-size:36px;font-weight:800;">{s.get('neutral',0)}%</div>
            </div>
            <div>
                <div style="color:#7ee0a0;font-size:14px;font-weight:600;">😊 Positive</div>
                <div style="color:#4ade80;font-size:36px;font-weight:800;">{s.get('positive',0)}%</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with t2:
            for seg in a.get("segments",[]):
                st.markdown(f"**{seg['name']}** — *{seg['size']}*")
                st.caption(seg["desc"]); st.markdown(f"⚠️ {seg['pain']}"); st.divider()
        with t3:
        ...
        st.markdown(f"""<div style="background:rgba(255,180,0,0.1);border:1px solid #ffb800;border-radius:12px;padding:14px 18px;margin-top:10px;box-shadow:0 4px 14px rgba(0,0,0,0.35);">
...
<p style="color:#fff;font-size:13px;margin:0;line-height:1.75;">{a.get("root_cause","")}</p>
        ...
        st.markdown(f"""<div style="background:#1a1a1a;border:1px solid #1DB954;border-radius:12px;padding:14px 18px;margin-top:10px;box-shadow:0 4px 14px rgba(0,0,0,0.35);">
